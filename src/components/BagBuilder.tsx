    ...
    <section ref={containerRef} id="estimator" className="relative z-40 py-24 lg:py-32 px-8 sm:px-12 lg:px-20 max-w-[1400px] mx-auto">
      ...
      <div className="builder-fade-up opacity-0 relative z-50 grid grid-cols-1 lg:grid-cols-[1.5fr_1fr_1fr] gap-8 lg:gap-12 mb-24 p-8 lg:p-12 rounded-[3px] bg-parchment border border-linen shadow-[0_10px_40px_rgba(0,0,0,0.02)]">
        ...
        <div className="flex flex-col justify-center gap-8 lg:pr-6">
          <div className="flex flex-col gap-2">
            <label className="text-[12px] tracking-[3px] uppercase font-medium text-teal">Event Type</label>
            <CustomSelect value={eventType} onChange={setEventType} options={EVENT_TYPES} placeholder="Select occasion..." />
          </div>
        </div>
        <div className="flex flex-col justify-center gap-8">
          <div className="flex flex-col gap-2">
            <label className="text-[12px] tracking-[3px] uppercase font-medium text-teal">Service Style</label>
      ...
      <div className="mb-12">
        <div className="flex items-center gap-4 mb-8">
          <h3 className="uppercase text-[11px] tracking-[3px] font-bold text-ink">Option A: Curated Packages</h3>
        ...
        <div className="builder-fade-up opacity-0 grid grid-cols-1 sm:grid-cols-3 gap-6">
          {PACKAGES.map((pkg) => (
            <div key={pkg.name} className="group relative p-8 rounded-[3px] bg-ink text-paper border border-white/10 transition-all duration-500 hover:-translate-y-2 overflow-hidden">
              ...
              <p className="text-[13px] text-paper/60 leading-relaxed mb-6 h-12 line-clamp-2">{pkg.desc}</p>
              <div className="flex items-baseline gap-1 mb-6">
                <span className="text-amber text-xl font-medium">${pkg.min}</span>
                <span className="text-paper/40 text-[10px] uppercase tracking-wider">Starting per head</span>
              </div>
              <button className="w-full py-3 border border-white/20 text-[10px] uppercase tracking-[2px] hover:bg-white hover:text-ink transition-colors duration-300">
      ...
      <div className="mb-12">
        <div className="flex items-center gap-4 mb-10">
          <h3 className="uppercase text-[11px] tracking-[3px] font-bold text-ink">Option B: Choose from the Menu!</h3>
          <div className="h-[1px] flex-1 bg-linen"></div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-12">
          {MENU.map((cat, i) => (
            <button key={cat.category} onClick={() => setActiveTab(i)} className={`px-6 py-2.5 rounded-full text-[10px] tracking-[1.5px] font-medium uppercase transition-all duration-300 ${activeTab === i ? "bg-teal text-white shadow-lg shadow-teal/20" : "bg-white text-teal border border-linen hover:border-teal/30"}`}>
              {cat.category}
            </button>
          ))}
        </div>
              ...
              <div key={item.id} className={`menu-item-card p-8 rounded-[3px] border transition-all duration-500 ${isSelected ? "bg-white border-teal shadow-xl" : "bg-white border-linen"}`}>
                ...
                <div className="flex items-center justify-between">
                   {isSelected ? (
                    ...
                    <button onClick={() => onAdd(item, MENU[activeTab].category, guestNum)} className="flex items-center gap-2 text-[10px] tracking-[1.5px] uppercase font-bold text-teal group">
      ...
      {bag.length > 0 && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] w-[calc(100%-40px)] max-w-[800px] bg-ink p-6 lg:p-8 flex items-center justify-between shadow-2xl border border-white/10 rounded-[4px]">
           <div className="hidden md:block">
             <p className="text-[10px] uppercase tracking-[2px] text-amber mb-1">{bagCount} Items Selected</p>
             <p className="font-serif text-paper text-2xl">${bagTotal.toLocaleString()}</p>
           </div>
           <div className="flex gap-4 w-full md:w-auto">
              <button onClick={onOpenBag} className="flex-1 md:flex-initial h-12 px-8 bg-teal text-white text-[10px] uppercase tracking-[2px] flex items-center justify-center gap-2">
