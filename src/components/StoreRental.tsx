...
export default function StoreRental() {
    ...
    useIsomorphicLayoutEffect(() => {
        ...
        const ctx = gsap.context(() => {
            gsap.fromTo(
                ...
                { y: "0%", rotate: 0, opacity: 1, duration: 1.2, stagger: 0.1, ease: "power4.out", scrollTrigger: { trigger: containerRef.current, start: "top 80%" } }
            );
            gsap.fromTo(
                ".sr-fade-up",
                { y: 30, opacity: 0 },
    ...
    return (
        <section ref={containerRef} className="py-24 lg:py-36 px-8 sm:px-12 lg:px-20 bg-ink text-cream">
            <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
                ...
                <div className="flex flex-col items-start">
                    ...
                    <h2 className="font-serif font-light text-paper tracking-tight flex flex-col gap-1 mb-6" style={{ fontSize: "clamp(38px, 4.5vw, 64px)", lineHeight: "1.05" }}>
                        <span className="overflow-hidden block py-1"><span className="sr-reveal-line block opacity-0">Hire the Lounge.</span></span>
                        ...
                        <ul className="flex flex-col gap-4">
                            {[
                                ...
                                <li key={item} className="flex items-center gap-4">
                                    <Check className="w-4 h-4 text-amber shrink-0" strokeWidth={2.5} />
                                    <span className="text-[16px] font-light text-cream/70">{item}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
