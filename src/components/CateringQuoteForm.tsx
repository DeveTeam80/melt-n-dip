...
export default function CateringQuoteForm({ bag, bagTotal, guests, eventType, serviceStyle }: Props) {
    const containerRef = useRef<HTMLElement>(null);
    const [form, setForm] = useState({ firstName: "", lastName: "", phone: "", email: "", details: "" });
    ...
    useIsomorphicLayoutEffect(() => {
        ...
        const ctx = gsap.context(() => {
            gsap.fromTo(".quote-fade",
                { y: 30, opacity: 0 },
                { y: 0, opacity: 1, duration: 1, stagger: 0.1, ease: "power3.out", scrollTrigger: { trigger: containerRef.current, start: "top 80%" } }
            );
        }, containerRef);
        return () => ctx.revert();
    }, []);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await fetch("/api/catering-enquiry", {
                ...
                body: JSON.stringify({
                    ...form, eventType, service: serviceStyle, guests,
                    items: bag,
                    estimatedTotal: bagTotal > 0 ? `$${bagTotal.toLocaleString()}` : undefined,
                }),
            });
            setSubmitted(true);
        } catch {
    ...
    const onMagLeave = (e: React.MouseEvent<HTMLElement>) => gsap.to(e.currentTarget, { x: 0, y: 0, duration: 0.8, ease: "elastic.out(1, 0.3)" });

    return (
        <section ref={containerRef} id="quote" className="relative z-40 py-24 lg:py-36 px-8 sm:px-12 lg:px-20 max-w-[1400px] mx-auto">
                    ...
                    {bag.length > 0 && guestNum > 0 && (
                        <div className="quote-fade opacity-0 mt-10 p-6 rounded-[3px] bg-teal-faint border border-teal-pale w-full">
                            ...
                            <div className="flex flex-col gap-3">
                                {bag.map(i => (
                                    <div key={i.id} className="flex justify-between pb-3 border-b border-teal-pale/30">
                                        <span className="text-[14px] text-ink font-light">{i.name} × {i.quantity}</span>
                                        <span className="text-[14px] text-teal font-medium">${(i.pricePerPerson * i.quantity * guestNum).toLocaleString()}</span>
                                    </div>
                                ))}
                            </div>
                            <div className="flex justify-between items-center pt-4">
                                <span className="text-[13px] font-semibold text-ink uppercase tracking-[1px]">Estimated Total</span>
                                <span className="text-[18px] font-bold text-teal">${bagTotal.toLocaleString()}</span>
                            </div>
                        </div>
                    )}
                </div>
                        ...
                        <div className="relative z-10 flex flex-col h-full">
                            ...
                            <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
                                ...
                                <div className="mt-4 relative z-0">
                                    <button type="submit" disabled={sending} onMouseMove={onMagMove} onMouseLeave={onMagLeave}
                                        className="cta-primary hover-target w-full flex items-center justify-center h-[56px] text-[15px]"
