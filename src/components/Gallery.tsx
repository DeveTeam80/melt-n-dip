...
export default function Gallery() {
    ...
    return (
        <section ref={sectionRef} className="relative bg-paper overflow-hidden h-screen flex flex-col justify-center">

            <div className="absolute top-12 md:top-24 left-8 sm:left-12 lg:left-20 z-10">
                <h2 className="gal-title opacity-0 font-serif font-light text-ink tracking-tight flex items-center gap-6" style={{ fontSize: "clamp(32px, 4vw, 56px)" }}>
            ...
            <div ref={scrollRef} className="flex gap-6 lg:gap-10 px-8 sm:px-12 lg:px-20 mt-16 lg:mt-24 h-[50vh] min-h-[400px]">
                {IMAGES.map((img, i) => (
                    <div
                        key={i}
                        className="relative h-full shrink-0 rounded-[3px] overflow-hidden group hover-target cursor-none"
                        style={{ width: i % 2 === 0 ? "35vw" : "25vw", minWidth: "280px", maxWidth: "600px" }}
                    >
                        <div
