...
export default function Cursor() {
  ...
  return (
    <>
      <div
        ref={dotRef}
        className="w-[6px] h-[6px] bg-teal rounded-full fixed pointer-events-none z-[9999] -translate-x-1/2 -translate-y-1/2 transition-transform duration-100 ease-linear"
      ...
      <div
        ...
        className={`fixed pointer-events-none z-[9998] -translate-x-1/2 -translate-y-1/2 rounded-full border transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          isHovered
            ? "w-[64px] h-[64px] border-teal bg-teal/5"
            : "w-[36px] h-[36px] border-teal/40"
        }`}
      />
    </>
  );
